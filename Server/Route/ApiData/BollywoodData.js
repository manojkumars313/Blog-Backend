const Bollywood = (req, res) => {
  res.json([
    {
      id: 1,
      photoUrl: "https://upload.wikimedia.org/wikipedia/en/0/0e/Salaar.jpg",
      // link: "https://www.google.com/search?client=firefox-b-d&q=ra+one",
      title: "Salaar",
      description:
        "Salaar is an upcoming Indian Telugu-language action thriller film written and directed by Prashanth Neel, and produced by Vijay Kiragandur under Hombale Films.",
      category: "Bollywood",
      date: "September 27 2023",
      paragraph:
        "Salaar is an upcoming Indian Telugu-language action thriller film written and directed by Prashanth Neel, and produced by Vijay Kiragandur under Hombale Films. It stars Prabhas in a dual role and as the titular charecter, along with Prithviraj Sukumaran, Shruti Haasan, and Jagapathi Babu. ",
    },
    {
      id: 2,
      photoUrl:
        "https://upload.wikimedia.org/wikipedia/en/d/d0/K.G.F_Chapter_2.jpg",
      // link: "https://www.google.com/search?client=firefox-b-d&q=ra+one",
      title: "KGF 2",
      description:
        "In the blood-soaked Kolar Gold Fields, Rocky's name strikes fear into his foes. While his allies look up to him, the government sees him as a threat to law and order. Rocky must battle threats from all sides for unchallenged supremacy.",
      category: "Bollywood",
      date: "APRIL 14 2022",
      paragraph:
        "Rocky successfully rises as the leader and saviour of the people of the Kolar Gold Fields. But, in his goal to fulfil his mother's wishes, Rocky must tackle Adheera, Inayat Khalil and Ramika Sen.Rocky successfully rises as the leader and saviour of the people of the Kolar Gold Fields. But, in his goal to fulfil his mother's wishes, Rocky must tackle Adheera, Inayat Khalil and Ramika Sen.",
    },
    {
      id: 3,
      photoUrl:
        "https://upload.wikimedia.org/wikipedia/en/7/75/Pushpa_-_The_Rise_%282021_film%29.jpg",
      // link: "https://www.google.com/search?client=firefox-b-d&q=kal+ho+na+ho",
      title: "Pushpa The Rise",
      description:
        "A labourer named Pushpa makes enemies as he rises in the world of red sandalwood smuggling. However, violence erupts when the police attempt to bring down his illegal business.",
      category: "Bollywood",
      date: "December 16 2021",
      paragraph:
        "A labourer named Pushpa makes enemies as he rises in the world of red sandalwood smuggling. However, violence erupts when the police attempt to bring down his illegal business.A labourer named Pushpa makes enemies as he rises in the world of red sandalwood smuggling. However, violence erupts when the police attempt to bring down his illegal business.",
    },
    {
      id: 4,
      photoUrl:
        "https://img1.hotstarext.com/image/upload/f_auto,t_vl/sources/r1/cms/prod/old_images/vertical/MOVIE/3314/1770003314/1770003314-v",
      // link: "https://www.google.com/search?client=firefox-b-d&q=kal+ho+na+ho",
      title: "M.S. Dhoni: The Untold Story",
      description:
        "M S Dhoni, a boy from Ranchi, aspires to play cricket for India. Though he initially tries to please his father by working for the Indian Railways, he ultimately decides to chase his dreams.",
      category: "Bollywood",
      date: "30 September 2016 (India)",
      paragraph:
        "M S Dhoni, a boy from Ranchi, aspires to play cricket for India. Though he initially tries to please his father by working for the Indian Railways, he ultimately decides to chase his dreams.M S Dhoni, a boy from Ranchi, aspires to play cricket for India. Though he initially tries to please his father by working for the Indian Railways, he ultimately decides to chase his dreams.",
    },
    {
      id: 5,
      photoUrl:
        "https://img1.hotstarext.com/image/upload/f_auto,t_vl/sources/r1/cms/prod/9219/1389219-v-9752e6e5bb92",
      // link: "https://www.google.com/search?client=firefox-b-d&q=ra+one",
      title: "Chhichhore",
      description:
        "A tragic incident forces Anirudh, a middle-aged man, to take a trip down memory lane and reminisce his college days along with his friends, who were labelled as losers.",
      category: "Bollywood",
      date: " 6 September 2019 (India)",
      paragraph:
        "A tragic incident forces Anirudh, a middle-aged man, to take a trip down memory lane and reminisce his college days along with his friends, who were labelled as losersA tragic incident forces Anirudh, a middle-aged man, to take a trip down memory lane and reminisce his college days along with his friends, who were labelled as losers.",
    },
    {
      id: 6,
      photoUrl:
        "https://upload.wikimedia.org/wikipedia/en/d/d0/K.G.F_Chapter_2.jpg",
      link: "https://www.google.com/search?client=firefox-b-d&q=ra+one",
      title: "KGF 2",
      description:
        "In the blood-soaked Kolar Gold Fields, Rocky's name strikes fear into his foes. While his allies look up to him, the government sees him as a threat to law and order. Rocky must battle threats from all sides for unchallenged supremacy.",
      category: "Bollywood",
      date: "APRIL 14 2022",
    },
  ]);
};
module.exports = Bollywood;
